...
class StreamAndMongoLogger:
    ...
    def __init__(
        ...
        formatter = logging.Formatter(
            "%(asctime)s-[%(name)s] - %(levelname)s - [%(source_file)s:%(line_number)d] - %(message)s"
        )
        ...
        stream_handler.setFormatter(
            logging.Formatter(
                "[%(name)s:%(levelname)s]-[%(source_file)s:%(line_number)d] - %(message)s"
            )
        )
    ...
    def info(self, message):
        ...
        lineno = frame.lineno
        self.logger.info(
            message, extra={"source_file": filename, "line_number": lineno}
        )
